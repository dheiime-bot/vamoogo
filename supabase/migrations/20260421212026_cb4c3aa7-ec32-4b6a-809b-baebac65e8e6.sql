...
-- 1) Atualiza driver_wallet_history para incluir o percentual da taxa em cada corrida
...
BEGIN
  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'fee_percent', CASE
        WHEN COALESCE(r.price, 0) > 0
        THEN ROUND((COALESCE(r.platform_fee, 0) / r.price * 100)::numeric, 2)
        ELSE NULL
      END,
      'category', r.category,
...
-- 2) Backfill: recalcula avaliação de todos os motoristas que têm rides avaliadas
DO $$
DECLARE _did uuid;
BEGIN
  FOR _did IN
    SELECT DISTINCT driver_id FROM public.rides
    WHERE driver_id IS NOT NULL AND rating IS NOT NULL
  LOOP
    PERFORM public.recalc_driver_rating(_did);
  END LOOP;

  -- Idem para passageiros
  FOR _did IN
    SELECT DISTINCT passenger_id FROM public.rides
    WHERE passenger_id IS NOT NULL AND driver_rating IS NOT NULL
  LOOP
    PERFORM public.recalc_passenger_rating(_did);
  END LOOP;
END $$;
