...
AS $$
    ...
    SELECT
      ...
      COALESCE(
        ...
        (
          ...
          ORDER BY ro.responded_at DESC NULLS LAST, ro.created_at DESC
          LIMIT 1
        )
  ...
  SELECT
    rd.resolved_driver_id AS user_id,
    ...
    d.pix_key_type
  FROM ride_driver rd
  LEFT JOIN public.drivers d ON d.user_id = rd.resolved_driver_id
  LEFT JOIN public.profiles p ON p.user_id = rd.resolved_driver_id
  LEFT JOIN LATERAL (
    SELECT dv.vehicle_brand, dv.vehicle_model, dv.vehicle_color, dv.vehicle_plate
    FROM public.driver_vehicles dv
    WHERE dv.driver_id = rd.resolved_driver_id
      AND dv.status = 'approved'
    ORDER BY dv.is_active DESC, dv.approved_at DESC NULLS LAST, dv.created_at DESC
    LIMIT 1
  ) v ON true
  WHERE rd.resolved_driver_id IS NOT NULL
    AND (
      rd.passenger_id = auth.uid()
      OR rd.resolved_driver_id = auth.uid()
      OR has_role(auth.uid(), 'admin'::app_role)
      OR is_master(auth.uid())
    )
    AND rd.status IN ('accepted'::ride_status, 'in_progress'::ride_status, 'completed'::ride_status);
$$;
