...
AS $function$
DECLARE
  requested_user_type text := COALESCE(NEW.raw_user_meta_data->>'user_type', 'passenger');
  safe_user_type public.user_type := 'passenger'::public.user_type;
  safe_app_role public.app_role := 'passenger'::public.app_role;
BEGIN
  IF requested_user_type = 'driver' THEN
    safe_user_type := 'driver'::public.user_type;
    safe_app_role := 'driver'::public.app_role;
  END IF;
  ...
  IF safe_user_type = 'driver'::public.user_type THEN
